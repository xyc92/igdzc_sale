...
from igdzc_sale.items import IgdzcSaleItem 
...
class igdzc_salespider(scrapy.Spider):
    ...
    custom_settings = {
        ...
        "ITEM_PIPELINES":{
            'igdzc_sale.pipelines.IgdzcSalePipeline': 300
        }
    }
    ...
    def parse_model(self,response):
        ...
        for dict in bodys:
            ...
            if 'averagePrice' in dict:
                HouseItem['averageprice']=dict['averagePrice'] #均价,int
            ...
            HouseItem['price']=dict['price']/10000 #价格，int
    ...
    def parse_detail(self,response):
        ...
        HouseItem['smallimgurls']=response.xpath('//*[@id="imgUl"]/li/img/@src').extract() #房源图片链接，array
        HouseItem['company']=response.xpath('//div[@class="img-right"]/dl[5]/dd/text()').extract()[0] #小区开发商，str
        HouseItem['avator']=response.xpath('//div[@class="broker-brief-details"]/span/a/img/@src').extract()[0] #经纪人头像图片链接，str
        if ifurl:
